...
from app.models import Customer, Restadmin, Items, Orders, Diginadmin, Comments
from app import app, db
from app.sentimentanalysis import model_predict,load_model
...
@app.route('/commentNext',methods = ['GET','POST'])
def commentNext():
	...
	if request.method == "GET":
		...
		senti=load_model(comment)
		print("cccccccccccccccccc")
		print(senti)
	
		cmail=session['cmail']
		type=senti
		# Commentscheck = Comments.query.filter(and_(Comments.cmail == cmail, Comments.rname == rname,Comments.comment==comment)).first()
        
		Comm = Comments(rname=request.form["rname"], comment=request.form["comment"], cmail=request.form["cmail"], type=type)
		db.session.add(Comm) 
		db.session.commit() 
...
def viewcomment():
	if not session.get('amail'):
		return redirect(request.url_root)
	amail=session['amail']
	comm = Comments.query.filter()

	# mycustomer=Customer.query.filter(Customer.cid==myorders.cid)
	# iuour = orders.query.join(items, orders.iid==items.iid).add_columns(users.userId, users.name, users.email, friends.userId, friendId).filter(users.id == friendships.friend_id).filter(friendships.user_id == userID).paginate(page, 1, False)
		
	return render_template('viewcomm.html', comm=comm)

@app.route('/uviewcomment',methods=['GET','POST'])
def uviewcomment():
	if not session.get('cmail'):
		return redirect(request.url_root)
	cmail=session['cmail']
	...
	return render_template('uviewcomm.html', comm=comm)
...
def usercomment():
	...
	customer  = Customer.query.filter(Customer.cmail == cmail).first()
	cid=customer.cid
	myorders = Items.query.filter()

	# mycustomer=Customer.query.filter(Customer.cid==myorders.cid)
	# iuour = orders.query.join(items, orders.iid==items.iid).add_columns(users.userId, users.name, users.email, friends.userId, friendId).filter(users.id == friendships.friend_id).filter(friendships.user_id == userID).paginate(page, 1, False)
		
	return render_template('comment.html',cusname=customer.cname,restadmin = customer.query.all(), myorders=myorders)
...
def restmenu():
	...
	items = Items.query.filter(Items.rid == restid).all()
	restad = Restadmin.query.filter(Restadmin.rid == restid).first()
	return render_template('restmenu.html',restad=restad, restadmin=items,items=items)
...
def payment():
	...
	for l in a:
		print(l)
	print(items.count(temp) for temp in a)
	x={temp:items.count(temp) for temp in a}
	print(len(x))
	print (x)
	#if len(x) > 1:
		#c=","
...
def restregisterbyadmin():
	...
	if restadmin:
		# return redirect(url_for('adminHome1'))		
		return render_template('adminhome.html', admsg="Canteen Already Registered...!")
		...
		# return redirect(url_for('adminHome1'))	
		return render_template('adminhome.html', ssmsg="Canteen Registered Succcessfully...!")
